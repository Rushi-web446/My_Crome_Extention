const platformClassMap = {
  "maang.in": "d-flex align-items-center flex-wrap gap-2 justify-content-between mb-2",
  "leetcode.com": "flex items-start justify-between gap-4",
  "codeforces.com": "title",
};
const bookmarkIconURL = chrome.runtime.getURL("assets/bookmark.jpg");


document.addEventListener("DOMContentLoaded", () => {
  console.log("\n\n\n\n drfgtvbyhnbgvfcdxfvgbhn \n\n\n");
...
function showMyBookMarkIcon() {
  ...
  const sectionWhereAddedButton = getClassNameAccordingPlatform();
  if (!sectionWhereAddedButton) return;

  const aboveElement = document.getElementsByClassName(sectionWhereAddedButton)[0];
  if (!aboveElement) {
    console.warn("Target DOM element not found for the given class.");
    return;
  }
...
async function saveProblemForFuture() {
  ...
  const isProblemAlreadyExist = bookmarkedProblems.some(
    (problem) => problem.problemUniqueId.toLowerCase() === problemUniqueId.toLowerCase(),
  );
...
function getClassNameAccordingPlatform() {
  const url = window.location.href;

  for (const domain in platformClassMap) {
    if (url.includes(domain)) {
      return platformClassMap[domain];
    }
  }
